...
if __name__ == "__main__":
    ...
    pupil_vine = v.CVine(pupil_model_data, torch.Tensor(data["X"]))
    ...
    for i, layer in enumerate(random_model_data):
        for j, cop_data in enumerate(layer):
            cop = cop_data.model_init(device_list[0]).marginalize(
                torch.Tensor(np.random.random(*(data["X"].shape)))
            )
            random_model_data[i][j] = cop
    random_vine = v.CVine(
        random_model_data, torch.Tensor(np.random.random(*(data["X"].shape)))
    )
