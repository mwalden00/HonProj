...
class CVine:
        ...
        for layer in self.layers:
            ...
            for model in layer:
                if model.theta.numel() > 0:
                    copula_model = bvcopula.MixtureCopula(
                        model.theta[..., input_idxs],
                    ...
                    # it is Independence
                    copula_model = bvcopula.MixtureCopula(
                        model.theta, model.mix[..., input_idxs], model.copulas
                    )
    ...
    def sample(self, sample_size=torch.Size([])):
        ...
        samples = torch.empty(size=samples_shape, device=self.device).uniform_(
            1e-4, 1.0 - 1e-4
        ...
        for copulas in self.layers[::-1]:
            ...
            new_layer = self._layer_transform(
                upper, samples[..., self.N - upper.shape[-1] - 1], copulas
            )
    ...
    def entropy(self, alpha=0.05, sem_tol=1e-3, mc_size=10000, v=False):
        ...
        conf = torch.erfinv(torch.tensor([1.0 - alpha], device=self.device))
        inputs = self.inputs.numel()
        sem = torch.ones(inputs, device=self.device) * float("inf")
        ent = torch.zeros(
            inputs, device=self.device
        )  # theta here must have dims: copula x batch dims
        var_sum = torch.zeros(inputs, device=self.device)
        log2 = torch.tensor([2.0], device=self.device).log()
        k = 0
        with torch.no_grad():
            while torch.any(sem >= sem_tol):
                ...
                samples = self.sample(
                    torch.Size([mc_size])
                )  # inputs (MC) x samples (MC) x variables
                ...
                assert torch.all(
                    logp.abs() != float("inf")
                )  # otherwise make masked tensor below
                ...
                sem = conf * (var_sum / (k * mc_size * (k * mc_size - 1))).pow(0.5)
                if v & (k % 10 == 0):
    ...
    def inputMI(
        ...
        # Gaussian confidence interval for sem_tol and level alpha
        conf = torch.erfinv(torch.tensor([1.0 - alpha])).to(self.device)
        ...
        log2 = torch.tensor([2.0]).log().to(self.device)
        k = 0
        inputs = self.inputs.numel()
        ...
        with torch.no_grad():
            while torch.any(sem >= sem_tol):
                ...
                samples = subvine.sample(
                    torch.Size([r_mc_size])
                )  # inputs (MC) x responses (MC) x variables
                samples = torch.einsum(
                    "ij...->ji...", samples
                )  # responses (MC) x inputs (MC) x variables
                # these are samples for p(r|s) for each s
                # size [responses(samples), stimuli(inputs), variables] = [r,s,v]
                logpRgS = (
                    subvine.log_prob(samples) / log2
                )  # dim=-2 aligns with the number of inputs in subvine
                ...
                samples = samples.reshape(
                    -1, samples.shape[-1]
                )  # (samples * Xs) x variables = [r*s, v]
                samples = samples.unsqueeze(
                    dim=-2
                )  # (samples * Xs) x 1 x 2 = [r*s, 1, v]
                samples = samples.expand(
                    [N, sR_mc_size, samples.shape[-1]]
                )  # (samples * Xs) x Xs x v
                ...
                while torch.any(
                    rR >= sem_tol_pr
                ):  # relative error of p(r) = absolute error of log p(r)
                    new_subset = torch.randperm(inputs)[
                        :sR_mc_size
                    ]  # permute samples & inputs
                    new_subvine = self.create_subvine(new_subset)
                    pRs = (
                        new_subvine.log_prob(samples)
                        .clamp(-float("inf"), max_log)
                        .exp()
                    )
                    ...
                    # Estimate standard error
                    var_sumR += (
                        ((pRs - pR.unsqueeze(-1)) ** 2).clamp(0, 1e2).sum(dim=-1)
                    )
                    semR = conf * (
                        var_sumR / (kR * sR_mc_size * (kR * sR_mc_size - 1))
                    ).pow(0.5)
                ...
                sem = conf * (var_sum / (k * N * (k * N - 1))).pow(0.5)
                if v:
                    print(
                        f"{Hrs.mean().item():.3},{Hr.mean().item():.3},{(Hrs.mean()-Hr.mean()).item():.3},\
                        {sem[0].max().item()/sem_tol:.3},{sem[1].max().item()/sem_tol:.3}"
                    )  # balance convergence rates
        assert torch.all(Hr == Hr)
        return (
            (Hrs - Hr),
            (sem[0] ** 2 + sem[1] ** 2).pow(0.5),
            Hrs,
            sem[1],
        )  # 2nd arg is an error of sum
