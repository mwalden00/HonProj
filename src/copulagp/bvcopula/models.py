...
class MultitaskGPModel(gpytorch.models.ApproximateGP):
    def __init__(
        self, num_dim, grid_bounds=(0, 1), prior_rbf_length=0.5, grid_size=None
    ):
        ...
        def _grid_size(num_dim):
            if num_dim < 4:
                ...
                grid_size = int(
                    conf.grid_size / int(math.sqrt(num_dim))
                )  # ~constant memory usage for covar
            return grid_size
        ...
        self.covar_module = gpytorch.kernels.ScaleKernel(
            gpytorch.kernels.RBFKernel(
                lengthscale_prior=lengthscale_prior, batch_shape=torch.Size([num_dim])
            ),
...
class Pair_CopulaGP:
    def __init__(
        self, copulas: list, device="cpu", grid_size=None, prior_rbf_length=0.5
    ):
        ...
        self.__gp_model = (
            MultitaskGPModel(
                self.__likelihood.f_size,
                grid_bounds=(0, 1),
                prior_rbf_length=prior_rbf_length,
                grid_size=grid_size,
            )
            .to(device=device)
            .float()
...
class Pair_CopulaGP_data:
    ...
    def model_init(self, device):
        ...
        likelihoods = MixtureCopula_Likelihood.deserialize(
            self.bvcopulas, just_likelihoods=True
        )
